blb_rf_imb=function(data.train,data.test,bag_num,M){
  gmean=c()
  a=Sys.time()
  CLASS_NAME <- "y"
  ...
  bag_count=nrow(data.train)/bag_num
  ...
  for (i in 1:bag_num){
    trees <- list()
    index_unique=c()
    pos_data=data.train[which(data.train$y=='yes'),]
    ...
    for(j in 1:M){
      ...
      traindata_a_tree=as.data.frame(traindata_a_tree[features])
      #用选定的说明变量创建训练数据
      tree <- new("decisionTree")
      tree@data.train  <- traindata_a_tree
